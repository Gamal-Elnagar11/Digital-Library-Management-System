...
namespace BookShopping_MVC.Controllers
{
    ...
    public class AdminOperationsController : Controller
    {
        ...
        public async Task<IActionResult> UpdateOrderStatus(int orderId)
        {
            ...
            var orderStatusList = (await _userOrderRepository.GetOrdersStatuses())
                                  .Select(orderStatus =>
                                  {
        ...
        public async Task<IActionResult> UpdateOrderStatus(UpdateOrderStatusModel data)
        {
            try
            {
                if (!ModelState.IsValid)
                {
                    data.OrderStatusList = (await
                        _userOrderRepository.GetOrdersStatuses()).Select(orderStatus =>
                        {
                            return new SelectListItem
                            {
                                Value = orderStatus.Id.ToString(),
            ...
            return RedirectToAction(nameof(UpdateOrderStatus)
                , new {orderId = data.OrderId});
                  // here if happen error will redirect to action updatePaymentStatus but it Get and send id to know how has error  
                  // orderId --> exist in new method
        }
